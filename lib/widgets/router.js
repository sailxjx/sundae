...
(function() {
  var Router, bucket, error, logger, path, router,
  ...
  Router = (function() {
    ...
    function Router(sundae) {
      ...
      this.rests = [];
      this.callback = function(err, $bucket) {
        var options;
        options = $bucket.get('options');
        if (err != null) {
          return _this._http500(err, $bucket.get('req'), $bucket.get('res'));
        } else {
    ...
    Router.prototype._applyCtrl = function(rest, options) {
      ...
      return this.app[method](route, function(req, res) {
        var $bucket;
        $bucket = bucket('rest');
        $bucket.set('req', req).set('res', res).set('func', func).set('ctrl', ctrl).set('options', options);
